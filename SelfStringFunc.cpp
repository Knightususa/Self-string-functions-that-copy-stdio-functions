...
int   IsSubStr   (const char* str, const char* substr, int i);
...
char* Selfstrstr (const char* str, const char* substr)
{
    ...
    {
        if(str[i] == substr[0]) isSubStr = IsSubStr(str, substr, i);
        if(isSubStr)
        {
            firstc = (char*) str + i;
            return firstc;
...
{
    int j = 0;
    while(j < Selfstrlen(substr) && i < Selfstrlen(str))
    {
        if(str[i] != substr[j]) return 0;
        i++;
        j++;
    }
    i--;
...
char* Selfstrdup (const char* str1)
{
    char* strcopy = (char*) calloc(Selfstrlen(str1), sizeof(char));
    int i = 0;
    while(str1[i] != '\0')
    {
        strcopy[i] = str1[i];
